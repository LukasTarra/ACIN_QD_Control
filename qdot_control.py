...
def plot_mean_dressed_population_trajectories(all_trajs, t_array, params_QD: ParametersQD):
    ...
    _, _, dressed_trans = create_dressed_energies_and_states(params_QD)
...
def create_jax_noise_traj_arrays(number_collapse: int, number_trajectories: int, number_steps: int, dt: float) -> tuple[jnp.ndarray, jnp.ndarray]:
    """
...
def main():
    ...
    input_array_abs = jnp.abs(input_array)

    # plot the real part of the control field
    plot_control_field(input_array_real, time_array, "Control Field (Real Part)")

    # simulate the system
    number_collapse = L_operators_j.shape[0]
    number_steps = len(time_array)
    dW_real_j, dW_imag_j = create_jax_noise_traj_arrays(number_collapse, number_trajectories_sim, number_steps, dt_sim)
    timer.start()
    ...
    # traj_orig_frame = U_rotating_frame_j @ traj
    traj_orig_frame = traj
    plot_mean_population_trajectories(traj_orig_frame, time_array)
    plot_mean_dressed_population_trajectories(traj_orig_frame, time_array, par_QD)
    # plot_mean_population_trajectories(traj, time_array)
